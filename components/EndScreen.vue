<template>
    <div class="flex items-center justify-center h-screen bg-gray-800 text-white">
      <div class="text-center">
        ...
        <ul class="px-10 py-10 bg-white rounded-md mb-4">
          ...
          <li class="mb-4" v-for="(child, index) in productsInCart.children" :key="index">
            <p class="text-slate-800"> {{child.userData.amount}}g {{ child.userData.name || 'Unbenannt' }}: </p>
        ...
        <button @click="restartFunction()" class="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded">
          Restart
        </button>
      </div>
    </div>
